...
notetypes-add-reverse-field = 反転カード追加（各裏面追記）
...
notetypes-hide-all-guess-one = マスク（のグループ）ごとにカード作成（他マスクも表示）
notetypes-hide-one-guess-one = マスク（のグループ）ごとにカード作成（他マスクは非表示）
