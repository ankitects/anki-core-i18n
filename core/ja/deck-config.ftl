...
deck-config-apply-all-parent-limits-tooltip =
    ...
    このオプションがオンの場合、各枚数の上限には、学習を開始する際に選択したデッキの設定とともに、そのデッキが属するメインデッキ（＝他のデッキのサブデッキでない、最上位のデッキ）についての設定も適用されます。「個々のサブデッキを選択して学習していきたいが、メインデッキ全体についての枚数上限を超えないようにしたい」という場合に役立つかもしれません。
