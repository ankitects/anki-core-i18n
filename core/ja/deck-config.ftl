...
deck-config-bury-interday-learning-siblings = 兄弟関係の習得中カード（前日以前からの持ち越し）を当日はパスする
...
deck-config-bury-interday-learning-tooltip = ある日に学習したカードに兄弟（同じノートから作られたカード〈例：裏表反転カード、別の箇所の穴埋め問題カード〉）がいて、その兄弟が`習得中`カードであり、前日以前から持ち越している場合であれば、その兄弟を表示する時間になったとしても当日中は表示を行わず、翌日に延期します。
deck-config-bury-priority-tooltip =
    Ankiは学習画面に表示するカードを準備する際、準備リストにまず「当日に学習を開始した習得中カード」を並べ、次に「前日以前から持ち越している習得中カード」、その次に復習カード、最後に新規カードを並べます。この並び方をもとに、どのカードを表示し、どのカードを延期するかが次のように決められます：
    ...
    - リスト内の兄弟どうしのうちで後ろに並んでいるカードの延期を解除したとしても、先に並んでいる別の種類のカードが代わりに延期されることはありません。例えばリスト内の、ある「前日以前から持ち越している習得中カード」とある新規カードが兄弟どうしの場合、たとえ新規カードの延期を解除（パスを取り消し）して学習したとしても、代わりに習得中カードが延期されてしまうことにはなりません。つまりこの場合は、兄弟どうしである習得中カードと新規カードの両方が同じ日に表示されることになります。
