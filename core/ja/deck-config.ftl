...
deck-config-compute-optimal-weights-tooltip =
    このパラメータの値は、そのプリセットを使用しているデッキでの復習履歴が1,000件以上に達してから「最適化」ボタンを押すと、別の値に置き換わります。その値は、あなたがその学習内容を無理なく長期的に記憶できるよう、復習履歴にもとづいて自動的に調整された値です。
    
    簡単なデッキと難しいデッキとでは、適したパラメータの値が異なるため、各デッキの難度が大きく異なる場合は、それぞれのデッキに別々のプリセットを使用することをおすすめします。
    
    パラメータの値の最適化を頻繁に行う必要はありません。数か月に一度で十分です。
    
    デフォルトでは、「最適化」によるパラメータの値は、現在のプリセットを使用しているすべてのデッキの復習履歴から計算されます。パラメータの値の最適化に使用するカードを変更したい場合は、パラメータの値を計算する前に、パラメータ欄の下にある、使用するカードの条件入力欄の内容を変更してください。
