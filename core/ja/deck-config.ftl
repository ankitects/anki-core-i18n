...
deck-config-leech-threshold-tooltip =
    復習カードに何度「`もう一度`」ボタンを押せば「苦戦（忘却多発）」カードと判定するか、を示す回数。
    
    苦戦（忘却多発）カードは、あなたの時間をたくさん消費しているカードです。苦戦（復習での忘却が多発している）カードと判定された場合、そのカードを再編集するか、削除するか、あるいは印象的な覚え方を工夫するなど、何らかの対策を立てることをお勧めします。
# See actions-suspend-card and scheduling-tag-only for the wording
deck-config-leech-action-tooltip =
    `タグのみ`：そのノートに「leech」というタグを付け、ポップアップを表示します。
