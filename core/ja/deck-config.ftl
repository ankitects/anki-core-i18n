...
deck-config-limit-new-bound-by-reviews = 一日の復習枚数の上限は、一日の新規カード導入枚数の上限に影響を及ぼします。例えば、一日の復習枚数の上限を200に設定し、期日を迎えたカードが190枚ある場合、その日に導入される新規カードは最大で10枚となります。また例えば、期日を迎えたカードの枚数が、一日の復習枚数の上限に達している場合、その日に新規カードは導入されません。
...
deck-config-new-insertion-order-tooltip = 新規カードを追加したときにそのカードに割り当てられる位置（新規カード番号）の決め方を選択できます。課題番号の数字が小さい順にカードは習得学習で表示されます。このオプションを変更すると、ただちに既存の新規カードの位置が更新されます。
...
deck-config-bury-interday-learning-siblings = 兄弟関係の習得中カード（かつ習得ステップ１日超）を当日はパスする
...
deck-config-bury-interday-learning-tooltip = ある日に学習したカードに兄弟（同じノートから作られたカード〈例：裏表反転カード、別の箇所の穴埋め問題カード〉）がいて、その兄弟が`習得中`カードである場合、習得ステップが１日より長い段階に達していれば、その兄弟を表示する時間になったとしても当日中は表示を行わず、翌日に延期します。
deck-config-bury-priority-tooltip =
    Ankiは学習画面に表示するカードを準備する際、準備リストにまず「間隔が１日未満の段階の習得中カード）を並べ、次に「間隔が１日以上の段階の習得中カード」、その次に復習カード、最後に新規カードを並べます。この並び方をもとに、どのカードを表示し、どのカードを延期するかが次のように決まります：
    
    - すべての延期オプションを有効になっている場合、リスト内の兄弟どうしのうち、最も先に並んでいるカードのみが表示されます。例えば、リスト内のある復習カードとある新規カードが兄弟どうしの場合、復習カードの方が先に並んでいるので、復習カードが表示され、新規カードの表示は翌日に延期されます。
    - リスト内の兄弟どうしのうちで後ろに並んでいるカードの延期を解除したとしても、先に並んでいる別の種類のカードが代わりに延期されることはありません。例えばリスト内の、ある「習得ステップが１日より長い段階の習得中カード」とある新規カードが兄弟どうしの場合、たとえ新規カードの延期を解除（パスを取り消し）して学習したとしても、代わりに習得中カードが延期されてしまうことにはなりません。つまりこの場合は、兄弟どうしである習得中カードと新規カードの両方が同じ日に表示されることになります。
