...
deck-config-new-card-sort-order-tooltip-2 =
    `カードタイプ順`: カードタイプの数字順にグループ分けをし、そのグループごとにカードを表示していきます。同じカードタイプどうしのカードは、集めたときの順序で表示されます。「兄弟関係のカードの延期」が無効になっている場合、例えば、カードタイプが「カード 1」の《表面→裏面》カードをすべて表示してから、カードタイプ「カード 2」の《裏面→表面》カードを表示していきます。兄弟関係のカード（＝同じノートから作られたカード）どうしを同じセッションで表示したいが、近づけすぎないようにもしたい、という場合に適した設定です。
