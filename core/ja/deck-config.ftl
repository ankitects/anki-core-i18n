...
deck-config-bury-priority-tooltip = うちAnkiは学習画面に表示するカードを準備する際、最初に「当日に学習を開始したばかりの習得中カード」を集め、次に「それ以外の習得中カード（＝前日以前に学習したことのある習得中カード）」、その次に復習カード、最後に新規カードを集めます。この集め方は、延期機能の働き方に影響を及ぼします。¶¶
