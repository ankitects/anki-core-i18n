...
deck-config-new-limit-tooltip = 一日に導入される新規カードの最大枚数。導入する新規カードの枚数が増加するにつれ、直近の復習量が一時的に増加し、過度な学習負荷につながります。そのため、一日あたりの新規カード出題枚数の上限は、一日あたりの復習カード出題枚数の上限の１０分の１以下となるように設定されています。
deck-config-review-limit-tooltip = 一日に表示される復習カードの最大枚数。
...
deck-config-limit-new-bound-by-reviews = 一日の復習上限の設定は、一日に導入される新規カードの枚数上限に影響します。例えば、一日の復習上限を200に設定し、期日を迎えたカードが190枚ある場合、最大10枚の新規カードが新たに導入されます。しかし、期日を迎えたカードの枚数が設定した上限を超えている場合、その日に新規カードは新たに導入されません。
