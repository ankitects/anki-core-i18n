...
deck-config-new-cards-ignore-review-limit-tooltip =
    このオプションがオフ（デフォルト）の場合、復習枚数の上限は、新規カード導入枚数も制限します。例えば、復習期日を迎えたカードの枚数が、復習枚数の上限に達している場合、その日に新規カードは導入されません。
    
    このオプションがオンの場合、新規カード導入枚数は、復習枚数の上限によって制限されないようになります。つまり、期日を迎えたカードの枚数が、復習枚数の上限に達しているとしても、それはそれとして、新規カードが導入されます。
...
deck-config-desired-retention-tooltip =
    デフォルト値の0.9では、あなたが復習の際に対象カードの90％で答えを忘れずに覚えていることを目標として復習スケジュールを組み立てます。
...
deck-config-sm2-retention-tooltip =
    FSRSに切り替える前の実際の正答保持率が0.9（＝90%）と大きく異なる場合にこの値を実態に即した値に調整することで、復習履歴が不明なカードについて、あなたの成績をAnkiがより適切に推定できるようになります。
    
    通常、復習履歴は、（ストレージの空き容量を増やすなどの理由で）意図的に削除でもしない限り失われることはないため、ほとんどのユーザーはこの値を調整する必要がありません。
deck-config-weights-tooltip =
    カードの復習スケジュールを調整する複数の変数。
    
    各パラメータには、最初はデフォルトの値が使用されています。
    
    このプリセットを使用しているデッキにおける復習履歴が1000件以上に達したら、「FSRSのパラメータを最適化」オプションによって、これまでのあなたの回答パフォーマンスに最適な値を算出して、それらの値を使用することができます。
...
deck-config-compute-optimal-retention-tooltip =
    このツールは、あなたがまだどのカードも学習していない状態で学習を開始すると仮定して、指定された学習プラン（枚数、日数、１日あたりの学習時間）に基づく正答保持率を推定します。
    
    推定される正答保持率は各項目に入力する値によって大きく変化します。推定値と0.9との差が著しく大きい場合は、予定の学習枚数に対して、予定の学習時間が少なすぎるか、または多すぎる可能性があります。
