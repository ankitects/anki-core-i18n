...
deck-config-limit-interday-bound-by-reviews = この枚数には、文字通りの復習カードだけでなく、その時点のステップが日をまたいでいる習得中（または再習得中）カードも含まれます。この枚数のカードが準備されるときは、ステップが日をまたいでいる習得中（または再習得中）カードが集められ、その後に復習カードが集められます。
...
deck-config-apply-all-parent-limits-tooltip =
    このオプションがオフ（デフォルト）の場合、各枚数の上限には、学習を開始する際に選択したデッキの設定と、それより下位の各デッキの設定がそれぞれ適用されます。
    
    このオプションがオンの場合、各枚数の上限には、最上位のデッキの設定と、それより下位の各デッキの設定がそれぞれ適用されます。「個々のサブデッキを選択して学習していきたいが、メインデッキ全体についての枚数上限を超えないようにしたい」という場合に適した方法です。
