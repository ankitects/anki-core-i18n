...
deck-config-desired-retention-tooltip2 = この値の調整中に表示される学習負荷についての比較情報（例：「変更後の学習負荷は、変更前(90%)の場合のおよそ0.87倍とな　ります」）の数値は比較的おおまかな概算によるものです。より精度の高い情報を確認したい場合は、シミュレータを使用してください。
