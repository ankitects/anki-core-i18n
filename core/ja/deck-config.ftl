...
deck-config-apply-all-parent-limits-tooltip =
    このオプションがオフ（デフォルト）の場合、各枚数の上限には、学習を開始する際に選択したデッキの設定および、その中のデッキ（サブデッキ）の設定がそれぞれ適用されます（なお、サブデッキの設定は、上位のデッキの設定の範囲内で適用されます）。つまり、学習を開始する際にサブデッキを選択すると、それより上位のデッキの設定は無視されます。
