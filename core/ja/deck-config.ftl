...
deck-config-learning-steps-tooltip =
    ...
    新規カードがあらかじめ最初のステップにある状態から習得学習が開始します。このステップの間隔は、新規カードまたは習得中カードの学習で「`やり直し`」ボタンを押した場合に使用されます。デフォルトでは1分となっています。
    
    最初のステップで「`正解`」ボタンを押すと、2番目のステップに進みます。デフォルトではこのステップの間隔は10分となっており、10分経過後にカードが再び表示されるようスケジュールが組まれます。
    
    すべてのステップで`正解`すると、そのカードは`復習カード`となり、翌日以降の別の日に再表示されるよう、スケジュールが組まれます。
...
deck-config-bury-review-tooltip =
    学習したカードと兄弟関係のカード（※）があり、そのカードが`復習`カードである場合、そのカードを表示する期日に達していても同じ日には表示を行わず、翌日に延期します。
    
    ※ 兄弟関係のカード：同じノートから作られたカード。例えば、裏表反転カード。
deck-config-bury-interday-learning-tooltip =
    学習したカードと兄弟関係のカード（※）があり、そのカードが`習得中`カードである場合、そのカードの現在のステップの間隔が日をまたいでいる場合に限り、同じ日には表示を行わず、翌日に延期します。
...
deck-config-bury-priority-tooltip =
    ...
    - リスト内の兄弟どうしのうちで後ろに並んでいるカードの延期を解除したとしても、先に並んでいる別の種類のカードが代わりに延期されることにはなりません。例えばリスト内の、ある復習カードとある新規カードが兄弟どうしの場合、たとえ新規カードの延期を解除して学習したとしても、代わりに復習カードが延期されてしまうことにはなりません。つまりこの場合は、兄弟どうしである復習カードと新規カードの両方が同じ日に表示されることになります。
...
deck-config-new-card-sort-order-tooltip-2 =
    `カードタイプ順→集めたときの順序`: カードタイプの数字順にカードを表示していきます。同じカードタイプどうしのカードは、集めたときの順序で表示されます。「兄弟関係のカードの延期」が無効になっている場合、例えば、カードタイプが「カード 1」の《表面→裏面》カードをすべて表示してから、カードタイプ「カード 2」の《裏面→表面》カードを表示していきます。兄弟関係のカード（＝同じノートから作られたカード）どうしを同じセッションで表示したいが、近づけすぎないようにもしたい、という場合に適した設定です。
    
    `集めたときの順序`: カードを集めたときのままの順序で表示します。「兄弟関係のカードの延期」が無効になっている場合、通常、兄弟関係のカードどうしは連続して表示されます。
    
    `カードタイプ順→ランダム`: 「カードタイプ順→集めたときの順序」と同じくカードタイプの数字順にグループ分けした後、各カードタイプのグループ内のカードをランダムな並びで表示します。カードを集めるときに「位置（新規カード番号）の昇順」で古いカードを優先的に集めた上で、この設定にすることによって、それらのカードをランダムな並びで表示し、兄弟関係のカードどうしを近づけすぎずに表示することができます。
...
deck-config-sort-order-template-then-gather = カードタイプ順→集めたときの順序
