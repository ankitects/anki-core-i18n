...
deck-config-compute-optimal-weights-tooltip =
    Ankiで1,000回以上復習したら、「最適化」ボタンを使って復習履歴を分析し、あなたの記憶力と学習内容に最適な重みを自動生成することができます。
    
    簡単なデッキと難しいデッキでは重みが異なるため、各デッキの難度が大きく異なる場合は、それぞれに別々のプリセットを使用することをお勧めします。
    
    重みを頻繁に最適化する必要はありません。数か月に 1 回で十分です。
    。ウェイトを最適化する必要はありません。
    数ヶ月に一度で十分です。
    ¶
    デフォルトでは、ウェイトは現在のプリセットを使用しているすべてのデッキのレビュー履歴から計算されます。以下のことができます。
    オプションで、ウェイトを計算する前の検索を調整することができます。
    ウェイトを最適化します。
