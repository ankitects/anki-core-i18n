...
deck-config-compute-optimal-weights-tooltip =
    このパラメータの値は、そのプリセットを使用しているデッキでの復習履歴が1,000件以上に達してから「最適化」ボタンを押すと、別の値に置き換わります。その値は、あなたが学習したこと（答え）を忘れる前のタイミングで復習できるよう、あなたのそれまでの回答にもとづいて自動的に調整された値です。
    ...
    デフォルトでは、「最適化」によるパラメータの値は、現在のプリセットを使用しているすべてのデッキのカードの復習履歴から計算されます。「最適化」の計算に使用するカードを変更したい場合は、「最適化」ボタンを押す前に、パラメータ欄の下にある、使用するカードの条件入力欄の内容を変更してください。
