...
statistics-true-retention-tooltip = FSRSを有効にしている場合、実際の正答率の値は、目標正答率の値に近くなることが期待されます。ただし、単日（「今日」「昨日」）での値は（回答数が比較的少ないため）変動しやすいので、実際の正答率が目標正答率に近いかどうかを確認するためには、「直近1か月」など、回答数が比較的多い期間での値に注目するほうが有意義でしょう。
