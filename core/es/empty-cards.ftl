empty-cards-for-note-type = Vaciar tarjetas para { $notetype }:
empty-cards-count-line = { $empty_count } de { $existing_count } tarjetas vacías ({ $template_names }).
empty-cards-window-title = Vaciar Tarjetas
empty-cards-preserve-notes-checkbox = Mantener notes sin cartas válidas
empty-cards-delete-button = Eliminar
empty-cards-not-found = No hay tarjetas vacías.
empty-cards-deleted-count =
    { $cards ->
        [one] { $cards } tarjeta eliminada.
       *[other] { $cards } tarjetas eliminadas.
    }
