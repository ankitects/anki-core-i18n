...
database-check-new-card-high-due = 已找到{ $count }张到期数字>=1,000,000的新卡片——考虑在浏览界面中重排它们的位置。
...
database-check-notetypes-recovered = 有一或多个笔记模板缺失。使用这些模板的笔记的模板已被 “db-check” 开头的笔记模板取代，但字段名称和卡片设计已丢失，因此建议您从自动备份中恢复。
