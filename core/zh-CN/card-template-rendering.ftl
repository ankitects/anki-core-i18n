...
card-template-rendering-front-side-problem = 正面模板存在问题：
card-template-rendering-back-side-problem = 反面模板存在问题：
...
# are missing.
card-template-rendering-empty-front = 卡片正面是空白。
card-template-rendering-missing-cloze =
    卡片上找不到挖空{ $number }。¶
    请添加一个挖空，或使用“工具>空卡片”来移除此卡片。
