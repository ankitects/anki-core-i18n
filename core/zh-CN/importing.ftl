...
importing-field-of-file-is = 文件中第<b>{ $val }</b>个字段：
...
importing-notes-that-could-not-be-imported = 因笔记类型已变更而无法导入的笔记：{ $val }
...
importing-the-first-field-of-the-note = 笔记类型的首字段必须匹配。
importing-the-provided-file-is-not-a = 此文件不是有效的. apkg文件。
importing-this-file-does-not-appear-to = 此文件不是有效的.apkg文件。如果您是从AnkiWeb下载的，请重新下载后重试。如问题仍然存在，请尝试使用其他浏览器。
importing-this-will-delete-your-existing-collection = 将会删除现有集合并将其替换为导入文件中的集合，是否确认?
