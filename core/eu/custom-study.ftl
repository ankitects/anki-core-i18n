custom-study-must-rename-deck = Lehenik, berrizendatu orain dagoen ikasketa pertsonalizaturako sorta.
custom-study-all-cards-in-random-order-dont = Txartel guztiak ausazko ordenan (ez birprogramatu)
custom-study-all-review-cards-in-random-order = Berrikusteko txartel guztiak ausazko ordenan
custom-study-cards = txartel
custom-study-cards-from-the-deck = txartel sortatik
custom-study-choose-tags = Aukeratu etiketak
custom-study-custom-study-session = Ikasketa-saio pertsonalizatua
custom-study-due-cards-only = Berrikusteko txartelak bakarrik
custom-study-increase-todays-new-card-limit = Handitu gaurko txartel berrien muga
custom-study-increase-todays-new-card-limit-by = Gehitu gaurko txartel berrien mugari
custom-study-increase-todays-review-card-limit = Handitu gaurko berrikusteko txartelen muga
custom-study-increase-todays-review-limit-by = Gehitu gaurko berrikusteko txartelen mugari
custom-study-new-cards-only = Txartel berriak bakarrik
custom-study-no-cards-matched-the-criteria-you = Eman duzun irizpidearekin bat datorren txartelik ez dago.
custom-study-ok = Ongi
custom-study-preview-new-cards = Aurreikusi txartel berriak
custom-study-preview-new-cards-added-in-the = Aurreikusi tarte honetan erantsitako txartel berriak: azken
custom-study-require-one-or-more-of-these = Gutxienez etiketa hauetako bat dutenak bakarrik:
custom-study-review-ahead = Berrikusi aurretiaz
custom-study-review-ahead-by = Berrikuspen aurrerakada :
custom-study-review-cards-forgotten-in-last = Berrikusi tarte honetan ahaztutako txartelak: azken
custom-study-review-forgotten-cards = Berrikusi ahaztutako txartelak
custom-study-select = Hautatu
custom-study-select-tags-to-exclude = Hautatu baztertzeko etiketak:
custom-study-selective-study = Hautatzeko errepasatzea
custom-study-study-by-card-state-or-tag = Kartaz edo etiketaz Ikasi
custom-study-available-child-count = ({ $count } azpisortetan)

## DEPRECATED - you do not need to translate these.

