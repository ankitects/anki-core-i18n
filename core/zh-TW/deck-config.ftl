...
deck-config-unable-to-determine-desired-retention = 無法計算出留存機率推薦最小值。
...
deck-config-predicted-minimum-recommended-retention = 留存機率推薦最小值：{ $num }
...
deck-config-reschedule-cards-warning =
    根據你的期望留存機率，這可能導致大量卡片到期，因此第一次從 FSRS 切換到 SM-2 時不建議使用。
...
deck-config-compute-optimal-retention-tooltip2 = 這個工具將假設你一開始有 0 張已學習的卡片，並將嘗試找出能使學習內容最多且耗時最少的期望留存機率。設定期望留存機率時可參考此數值。若你不在乎多花時間學習，可以透過提高期望留存機率來提升記憶效果。期望留存機率低於最小值只會增加你的工作量而沒有好處，因此不建議設定過低。
deck-config-compute-optimal-retention-tooltip3 = 這個工具將假設你一開始有 0 張已學習的卡片，並將嘗試找出能使學習內容最多且耗時最少的期望留存機率。為了準確模擬學習過程，此功能需要至少 400 次複習。設定期望留存機率時可參考計算出的數值。若你不在乎多花時間學習，可以透過提高期望留存機率來提升記憶效果。期望留存機率低於最小值會導致遺忘率過高，從而加大你的工作量，因此不建議設定過低。
