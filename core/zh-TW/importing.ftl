...
importing-by-default-anki-will-detect-the = 根據預設，Anki 會自動偵測欄位間的間隔字元，如定位字元 (tab) 或逗號。假如 Anki 偵測錯誤的話，你可以在此輸入。用 \t 來代表 tab。
importing-change = 更改
importing-colon = 冒號
importing-comma = 逗號
...
importing-this-will-delete-your-existing-collection = 這樣會刪除原本的集合，資料將被正在匯入的檔案取代。確定嗎？
