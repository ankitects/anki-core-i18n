...
importing-field-of-file-is = 檔案的第 <b>{ $val }</b> 個欄位:
importing-fields-separated-by = { $val } 分隔各欄位
importing-file-version-unknown-trying-import-anyway = 檔案版本未知，嘗試繼續匯入。
importing-first-field-matched = 符合第一個欄位: { $val }
...
importing-notes-that-could-not-be-imported = 因筆記類型已變更而無法匯入的筆記: { $val }
importing-notes-updated-as-file-had-newer = 因檔案有新版本而被更新的筆記: { $val }
...
importing-rows-had-num1d-fields-expected-num2d = 「%(row)s」行有 %(num1)d 個欄位，預期 %(num2)d
importing-selected-file-was-not-in-utf8 = 已選取的檔案並非 UTF-8 格式，請參閱用戶手冊的匯入部分。
importing-semicolon = 分號
importing-skipped = 已略過
importing-supermemo-xml-export-xml = Supermemo XML 匯出檔 (*.xml)
importing-tab = 分頁
importing-tag-modified-notes = 已修改標籤的筆記:
...
importing-this-file-does-not-appear-to = 此檔案看來不是有效的 .apkg 檔，如果您是從 AnkiWeb 下載檔案後收到此錯誤訊息，那有可能是下載失敗。請再試一次，如果又發生錯誤，請換另一個網頁瀏覽器再試一次。
importing-this-will-delete-your-existing-collection = 這樣會刪除您原本的集合，並以現在要匯入的檔案覆蓋之。您確定嗎？
