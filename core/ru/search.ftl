...
search-unknown-escape = не определена последовательность выхода { $val }`. Если вы хотите искать именно обратный слэш `\`, добавьте еще один обратный слэш: `\\`.
...
search-view-in-browser = Смотреть в поиске
