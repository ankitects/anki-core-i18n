decks-add-new-deck-ctrlandn = Добавить новую колоду (Ctrl+N)
decks-are-you-sure-you-wish-to = Вы уверены, что хотите удалить { $val }?
decks-build = Сборка
decks-cards-selected-by = карт, выбранных
decks-create-deck = Создать колоду
decks-custom-steps-in-minutes = Заданные шаги (мин.)
decks-deck = Колода
decks-decreasing-intervals = От большего интервала к меньшему
decks-delete-deck = Удалить колоду
decks-enable-second-filter = Использовать второй фильтр
decks-filter = Фильтр:
decks-filter-2 = Фильтр 2
decks-get-shared = Скачать колоду
decks-import-file = Импорт файла
decks-increasing-intervals = По возрастанию интервалов
decks-latest-added-first = Сначала последняя добавленная
decks-limit-to = Количество
decks-minutes = мин.
decks-most-lapses = Наиболее забываемые
decks-name-for-deck = Название колоды:
decks-new-deck-name = Имя новой колоды:
decks-no-deck = [нет колоды]
decks-oldest-seen-first = Сначала просмотренные давно
decks-order-added = В порядке добавления
decks-order-due = По расписанию повторения
decks-please-select-something = Выберите что-нибудь.
decks-random = В случайном порядке
decks-relative-overdueness = Относительная просроченность
decks-repeat-failed-cards-after = Повторить забытые карточки после
decks-reschedule-cards-based-on-my-answers = Перепланировать карточки на основе моих ответов в этой колоде
decks-study = Учить
decks-study-deck = Учить колоду
decks-the-provided-search-did-not-match = Нет карточек, удовлетворяющих условиям поиска. Желаете задать новые?
decks-it-has-card = { $count ->
    [one]  Содержит { $count } карточку.
    [few]  Содержит { $count } карточки.
   *[other]  Содержит { $count } карточек.
  }
