...
scheduling-graduating-interval = Интервал для «Хорошо»
...
scheduling-maximum-reviewsday = Максимум повторений в день
...
scheduling-parent-limit = (лимит у вышестоящей: { $val })
...
scheduling-show-new-cards-after-reviews = Показывать новые карточки после повторений
scheduling-show-new-cards-before-reviews = Показывать новые карточки перед повторениями
scheduling-show-new-cards-in-order-added = Показывать новые карточки в порядке их добавления
scheduling-show-new-cards-in-random-order = Показывать новые карточки в случайном порядке
scheduling-starting-ease = Исходная лёгкость
