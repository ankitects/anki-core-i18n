change-notetype-current = Текущая
change-notetype-new = Новая
change-notetype-will-discard-content = Удалит содержание следующих полей:
change-notetype-will-discard-cards = Удалит следующие карточки:
change-notetype-fields = Поля
change-notetype-templates = Шаблоны
change-notetype-to-from-cloze =
    При изменении типа записи с или на "Заполнить пробелы", номера карточек не меняются.¶
    ¶
    Если при переходе на обычный тип заметок пробелов больше, 
    чем доступных шаблонов карточек, все дополнительные карточки будут удалены.
