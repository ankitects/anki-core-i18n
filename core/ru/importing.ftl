...
importing-first-field-matched = Подходящее первое поле: { $val }
importing-identical = Одинаковые
importing-ignore-field = Игнорировать поле
importing-ignore-lines-where-first-field-matches = Игнорировать строки, если существует запись с таким же первым полем
importing-ignored = <проигнорировано>
...
importing-tag-modified-notes = Отметить изменённые записи:
importing-text-separated-by-tabs-or-semicolons = Текст, разделённый символами табуляции или точками с запятой (*)
