preferences-automatically-sync-on-profile-openclose = سەپلىمە ھۆججەتنى ئۆزلۈكىدىن قەدەمداشلاشنى ئاچ/ياپ
preferences-backups = زاپاس
preferences-change-deck-depending-on-note-type = كارتا قوشقاندا كارتا تۈرىگە ئاساسەن دەستە تاللىنىدۇ
preferences-changes-will-take-effect-when-you = ئۆزگىرىش Anki قايتا قوزغالغاندا ئاندىن ئۈنۈمىنى كۆرسىتىدۇ.
preferences-hours-past-midnight = سائەت
preferences-language = تىل
preferences-interrupt-current-audio-when-answering = كارتىغا جاۋاب بېرىۋاتقاندا نۆۋەتتىكى ئۈن قويۇشنى توختىتىدۇ
preferences-learn-ahead-limit = ئالدىن ئۆگىنىش چېكى
preferences-mins = مىنۇت
preferences-network = قەدەمداشلاش
preferences-next-day-starts-at = ئەتە باشلايدىغان ۋاقىت
preferences-media-is-not-backed-up = ۋاسىتە زاپاسلانمايدۇ. بىخەتەرلىك ئۈچۈن Anki قىسقۇچنى قەرەللىك زاپاسلاڭ.
preferences-on-next-sync-force-changes-in = كېيىنكى قېتىم قەدەمداشلىغاندا، مەجبۇرىي بىر يۆنىلىشتە ئۆزگەرتىدۇ
preferences-paste-clipboard-images-as-png = چاپلاش تاختىسىدىكى سۈرەتنى PNG پىچىمىدا چاپلايدۇ
preferences-paste-without-shift-key-strips-formatting = ئەگەر چاپلىغاندا shift كۇنۇپكىسى بېسىلمىسا تېكىست پىچىمىنى تازىلايدۇ
preferences-generate-latex-images-automatically = LaTeX سۈرىتى ھاسىللايدۇ (بىخەتەرلىك خەتىرى بار)
preferences-latex-generation-disabled = LaTeX سۈرەت ھاسىللاش مايىللىقتا چەكلەنگەن.
preferences-periodically-sync-media = ۋاسىتە ھۆججەتنى قەرەللىك قەدەمداشلايدۇ
preferences-please-restart-anki-to-complete-language = تىل ئالماشتۇرۇشنى تاماملاشتا Anki نى قايتا قوزغىتىڭ.
preferences-preferences = مايىللىق
preferences-scheduling = كۈنتەرتىپكە تىزىۋاتىدۇ
preferences-show-learning-cards-with-larger-steps = تەكرارلاشتىن ئىلگىرى كۆرسىتىش جەريانى ئۇزۇنراق ئۆگىنىۋاتقان كارتا
preferences-show-next-review-time-above-answer = جاۋاب توپچە ئۈستىدە كېيىنكى قېتىم تەكرارلاش ۋاقتىنى كۆرسىتىدۇ
preferences-spacebar-rates-card = بوشلۇق (ياكى enter) كۇنۇپكىسى كارتىغا جاۋا بېرىدۇ
preferences-show-play-buttons-on-cards-with = كارتىدا ئۈن قويۇش توپچەنى كۆرسىتىدۇ
preferences-show-remaining-card-count = قېپقالغان كارتا سانىنى كۆرسىتىدۇ
preferences-some-settings-will-take-effect-after = بەزى تەڭشەك Anki قايتا قوزغالغاندىن كېيىن ئۈنۈمىنى كۆرسىتىدۇ.
preferences-tab-synchronisation = قەدەمداشلاش
preferences-synchronize-audio-and-images-too = ئۈن ۋە سۈرەتنىمۇ قەدەمداشلايدۇ
preferences-login-successful-sync-now = مۇۋەپپەقىيەتلىك ھالدا تىزىمغا كىردى. مايىللىقنى ساقلاپ، دەرھال قەدەمداشلامدۇ؟
preferences-timebox-time-limit = بەلگىلەنگەن ۋاقىت خاتىرىلىگۈچ ۋاقىت چېكىدە ئۆگىنىدىغان كارتا سانىنى ئەسكەرتىدۇ
preferences-user-interface-size = ئىشلەتكۈچى ئارايۈز چوڭلۇقى
preferences-when-adding-default-to-current-deck = كارتا قوشقاندا، كۆڭۈلدىكى ئەھۋالدا نۆۋەتتىكى دەستىگە قوشىدۇ
preferences-you-can-restore-backups-via-fileswitch = ھۆججەت › تەرجىمىھال ئالماشتۇر ئارقىلىق زاپاسنى ئەسلىگە كەلتۈرەلەيسىز.
preferences-legacy-timezone-handling = كونا ۋاقىت رايونىنى بىر تەرەپ قىلىش ئۇسۇلى (مەسىلە بار، ئەمما <= 2.14  دىن تۆۋەن نەشرى چوقۇم  ئىشلىتىشى كېرەك.)
preferences-default-search-text = كۆڭۈلدىكى ئىزدەش تېكىستى
preferences-default-search-text-example = مەسىلەن «deck:current»
preferences-theme = ئۆرنەك
preferences-theme-follow-system = سىستېمىغا ئەگەش
preferences-theme-light = يورۇق
preferences-theme-dark = قاراڭغۇ
preferences-v3-scheduler = V3 كۈنتەرتىپلىگۈچنى قوزغات
preferences-updates = يېڭىلانما
preferences-check-for-updates = پىروگرامما يېڭىلىنىشىنى تەكشۈرىدۇ
preferences-check-for-addon-updates = قوشۇلما يېڭىلانمىسىنى تەكشۈر
preferences-ignore-accents-in-search = ئىزدىگەندە ئۇرغۇغا پەرۋا قىلمايدۇ (ئاستاراق)
preferences-backup-explanation = Anki توپلىمىڭىزنى ئۆزلۈكىدىن زاپاسلايدۇ. Anki دىسكا بوشلۇقىنى تېجەش ئۈچۈن زاپاسلانغان ۋاقتى 2 كۈندىن ئاشقان زاپاسنى ئۆچۈرىدۇ.
preferences-daily-backups = ساقلاپ قالىدىغان كۈندىلىك زاپاس سانى:
preferences-weekly-backups = ساقلاپ قالىدىغان ھەپتىلىك زاپاس سانى:
preferences-monthly-backups = ساقلاپ قالىدىغان ئايلىق زاپاس سانى:
preferences-minutes-between-backups = ئاپتوماتىك زاپاسلاش ئارىلىقى بىرلىكى مىنۇت:
preferences-reduce-motion = جانلاندۇرۇمنى ئازايتىدۇ
preferences-reduce-motion-tooltip = ئىشلەتكۈچى ئارايۈزىنىڭ بارلىق جانلاندۇرۇم ۋە ئالمىشىش ئۈنۈمىنى چەكلەيدۇ
preferences-custom-sync-url = ئۆزى باشقۇرىدىغان قەدەمداش مۇلازىمېتىر
preferences-custom-sync-url-disclaimer = ئالىي ئىشلەتكۈچىلەر ئۈچۈن - قوللانمىنى ئىشلىتىڭ
preferences-hide-top-bar-during-review = تەكرارلاۋاتقاندا چوققا بالداقنى يوشۇرىدۇ
preferences-hide-bottom-bar-during-review = تەكرارلاۋاتقاندا ئاستى بالداقنى يوشۇرىدۇ
preferences-always = ھەمىشە
preferences-full-screen-only = پۈتۈن ئېكرانلا
preferences-appearance = كۆرۈنۈش
preferences-general = ئادەتتىكى
preferences-style = ئۇسلۇب
preferences-review = تەكرار
preferences-answer-keys = جاۋاب كۇنۇپكا
preferences-distractions = بىزارلىقتىن خالىي
preferences-minimalist-mode = ئىخچام ھالىتى
preferences-minimalist-mode-tooltip = ئارايۈزنى تېخىمۇ ئىخچام/ئاددىيلاشتۇرىدۇ
preferences-editing = تەھرىرلەش
preferences-browsing = كۆز يۈگۈرتۈش
preferences-default-deck = كۆڭۈلدىكى دەستە
preferences-account = AnkiWeb ھېساباتى
preferences-note = ئىزاھات
preferences-scheduler = كۈنتەرتىپلىگۈچ
preferences-user-interface = ئىشلەتكۈچى ئارايۈزى
preferences-import-export = ئەكىر/چىقار
preferences-network-timeout = تور ۋاقىت ھالقىش
preferences-reset-window-sizes = كۆزنەك چوڭلۇقىنى ئەسلىگە قايتۇر
preferences-reset-window-sizes-complete = كۆزنەك چوڭلۇقى ۋە ئورنى ئەسلىگە قايتۇرۇلدى.
preferences-shortcut-placeholder = ئىشلەتمىگەن قىسقا يول كۇنۇپكىسى كىرگۈزۈلىدۇ ياكى بوش قالدۇرۇلسا چەكلەيدۇ.
preferences-third-party-services = ئۈچىنچى تەرەپ مۇلازىمەتلەر
preferences-ankihub-not-logged-in = نۆۋەتتە AnkiHub قا تىزىمغا كىرمىگەن.
preferences-ankiweb-intro = AnkiWeb كارتا سانلىق مەلۇماتلىرىڭىزنىڭ ئۈسكۈنىلىرىڭىز ئارىسىدا قەدەمداشلاش ۋە ئەگەر ئۈسكۈنىڭىز بۇزۇلۇپ كەتسە ياكى يوقاپ كەتسە سانلىق مەلۇماتلىرىڭىزنى ئەسلىگە كەلتۈرۈشنى قوللايدىغان ھەقسىز مۇلازىمەت.
preferences-ankihub-intro = AnkiHub ھەمكارلىقتىكى دەستە تەھرىرلەش ۋە قوشۇمچە ئۆگىنىش قوراللىرى بىلەن تەمىنلەيدۇ. بەزى ئىقتىدارنى ئىشلىتىش ئۈچۈن ھەقلىق مۇشتەرى بولۇش زۆرۈر.
preferences-third-party-description = ئۈچىنچى تەرەپ مۇلازىمىتى Anki بىلەن مۇناسىۋەتسىز ھەمدە قوللىمايدۇ. بۇ مۇلازىمەتلەرنى ئىشلىتىش ئۈچۈن ھەق تۆلىشىڭىز مۇمكىن.
preferences-experimental-features-tab = تەجرىبە

## URL scheme related

preferences-url-schemes = تور ئادرېس كېلىشىمى
preferences-url-scheme-prompt = { preferences-url-schemes } يول قويدى (بوشلۇقتا ئايرىلىدۇ):
preferences-url-scheme-warning =
    ئۇلانما «{ $link }» نى ئېچىشقا ئۇرۇنۇش توسۇلدى، بۇ مەشغۇلاتنىڭ بىخەتەرلىك خەتىرى بار.
    
    ئەگەر دەستە يازغۇچىغا ئىشىنىپ، مەشغۇلاتنى داۋاملاشتۇرماقچى بولسىڭىز، «{ $scheme }» نى يول قويىدىغان { preferences-url-schemes } غا قوشالايسىز.
preferences-url-scheme-allow-once = بىر قېتىملا يول قوي
preferences-url-scheme-always-allow = ھەمىشە يول قوي

## NO NEED TO TRANSLATE. This text is no longer used by Anki, and will be removed in the future.

preferences-basic = ئاساسى
preferences-reviewer = تەكرارلىغۇچى
preferences-media = ۋاسىتە
preferences-not-logged-in = نۆۋەتتە AnkiWeb گە تىزىمغا كىرمىگەن.
