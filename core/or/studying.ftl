studying-again = ପୁଣି
studying-all-buried-cards = ସମସ୍ତ ସ୍ଥଗିତ ପତ୍ର
studying-audio-5s = ଅଡ଼ିଓ -5ସେ
studying-audio-and5s = ଅଡ଼ିଓ +5ସେ
studying-buried-siblings = ସ୍ଥଗିତ ହୋଇଥିବା ଭାଇଭଉଣୀ
studying-bury = ସ୍ଥଗିତ ରଖନ୍ତୁ
studying-bury-card = ପତ୍ର ସ୍ଥଗିତ କରନ୍ତୁ
studying-bury-note = ନୋଟ୍ ସ୍ଥଗିତ କରନ୍ତୁ
studying-card-suspended = ପତ୍ରଟିକୁ ନିଲମ୍ବିତ କରି ଦିଆଗଲା
studying-card-was-a-leech = ପତ୍ର ଏକ ଜୋକ (leech) ଥିଲା।
studying-cards-buried =
    { $count ->
        [one] { $count }ଟିଏ ପତ୍ର ସ୍ଥଗିତ କରାଗଲା।
       *[other] { $count }ଟି ପତ୍ର ସ୍ଥଗିତ କରାଗଲା।
    }
studying-cards-will-be-automatically-returned-to = ଆପଣ ସମୀକ୍ଷା କରିବା ପରେ ପତ୍ରଗୁଡ଼ିକ ସ୍ୱତଃସ୍ପୃତ ଭାବେ ସେମାନଙ୍କର ମୂଳ ଡେକ୍ କୁ ଫେରସ୍ତ ହେବେ।
studying-continue = ଜାରି ରଖନ୍ତୁ
studying-delete-note = ନୋଟ୍ ଅପସାରଣ କରନ୍ତୁ
studying-deleting-this-deck-from-the-deck = ଡେକ୍ ତାଲିକାରୁ ଏହି ଡେକ୍ ବିଲୋପ କରିବା ଦ୍ୱାରା ଅବଶିଷ୍ଟ ସମସ୍ତ ପତ୍ର ସେମାନଙ୍କର ମୂଳ ଡେକ୍ କୁ ଫେରିବେ।
studying-easy = ସହଜ
studying-edit = ସମ୍ପାଦନା
studying-empty = ଖାଲି
studying-finish = ସମାପ୍ତ
studying-flag-card = ପତ୍ରକୁ ଏକ ପତାକା ନ୍ୟସ୍ତ କରନ୍ତୁ
studying-good = ଭଲ
studying-hard = କଠିନ
studying-it-has-been-suspended = ଏହାକୁ ନିଲମ୍ବିତ ରଖାଯାଇଛି।
studying-manually-buried-cards = ହସ୍ତକୃତ ଭାବେ ସ୍ଥଗିତ ହୋଇଥିବା ପତ୍ର
studying-mark-note = ନୋଟ ଚିନ୍ହିତ କରନ୍ତୁ
studying-more = ଅଧିକ
studying-no-cards-are-due-yet = ଏପର୍ଯ୍ୟନ୍ତ କୌଣସି ପତ୍ର ବାକି ନାହିଁ।
studying-note-suspended = ନୋଟଟିକୁ ନିଲମ୍ବିତ କରି ଦିଆଗଲା
studying-pause-audio = ଅଡ଼ିଓ ବିରତ କର
studying-please-run-toolsempty-cards = ଦୟାକରି ଉପକରଣଗୁଡ଼ିକ> ଖାଲି ପତ୍ରଗୁଡ଼ିକ ଚଲାନ୍ତୁ।
studying-record-own-voice = ନିଜ ସ୍ୱରକୁ ରେକର୍ଡ କରନ୍ତୁ
studying-replay-own-voice = ନିଜ ସ୍ୱର ପୁନଃଚାଳନ କରନ୍ତୁ
studying-show-answer = ଉତ୍ତର ଦେଖନ୍ତୁ
studying-study-now = ବର୍ତ୍ତମାନ ଅଧ୍ୟୟନ କରନ୍ତୁ
studying-suspend = ନିଲମ୍ବନ କରନ୍ତୁ
studying-suspend-note = ନୋଟ୍ ନିଲମ୍ବିତ କରନ୍ତୁ
studying-this-is-a-special-deck-for = ସାଧାରଣ ଅନୁସୂଚୀ ବାହାରେ ଅଧ୍ୟୟନ କରିବା ପାଇଁ ଏହା ଏକ ସ୍ୱତନ୍ତ୍ର ଡେକ୍।
studying-to-review = ସମୀକ୍ଷା କରିବାକୁ
studying-type-answer-unknown-field = ଉତ୍ତର ଟାଇପ୍ କରନ୍ତୁ: ଅଜ୍ଞାତ କ୍ଷେତ୍ର { $val }
studying-unbury = ସ୍ଥଗିତରୁ ଫେରାଇ ଆଣ
studying-what-would-you-like-to-unbury = ଆପଣ କ'ଣ ସ୍ଥଗିତରୁ ଫେରାଇ ଆଣିବାକୁ ଚାହାଁନ୍ତି?
studying-you-havent-recorded-your-voice-yet = ଆପଣ ଏପର୍ଯ୍ୟନ୍ତ ନିଜ ସ୍ୱର ରେକର୍ଡ କରିନାହାଁନ୍ତି।
studying-minute =
    { $count ->
        [one] { $count } ମିନିଟ୍।
       *[other] { $count } ମିନିଟ୍।
    }
studying-note-and-its-card-deleted =
    { $count ->
        [one] ନୋଟ୍ ଏବଂ ଏହାର { $count }ଟିଏ ପତ୍ର ବିଲୋପ ହୋଇଛି।
       *[other] ନୋଟ୍ ଏବଂ ଏହାର { $count }ଟି ପତ୍ର ବିଲୋପ ହୋଇଛି।
    }
